...
public class Customer extends Person {
    ...
    protected void inputUsageTime() throws IOException {
        ...
        while (true) {
            System.out.printf("%s님 통화시간 입력 : ", super.getName());
            usageTime = keyboard.nextInt();
            if (usageTime >= 0)
                break;
            else{
                System.err.print("ERROR : 통화시간 오류");
                System.in.read();
            }
        }
    }
    ...
    protected void inputMessage() throws IOException {
        Scanner keyboard = new Scanner(System.in);
        while (true) {
            System.out.printf("%s님 문자 건수 입력 : ", super.getName());
    ...
    public String toString() {
        return String.format("%s %15s %,7d초 %,7d건 %6s %,10d원 %,6d원 %,10d원",
